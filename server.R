...
function(input, output, session) {
  ...
  observeEvent(input$generate, {
    ...
    src5 <- normalizePath('report_code_container.Rmd') 
    src6 <- normalizePath('report_code.Rmd') 
    ...
    file.copy(src5, 'report_code_container.Rmd', overwrite = TRUE)
    file.copy(src6, 'report_code.Rmd', overwrite = TRUE)
